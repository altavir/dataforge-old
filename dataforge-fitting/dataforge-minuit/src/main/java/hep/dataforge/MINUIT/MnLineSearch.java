...
abstract class MnLineSearch {
    ...
    static MnParabolaPoint search(MnFcn fcn, MinimumParameters st, RealVector step, double gdel, MnMachinePrecision prec) {
        double overal = 1000.;
        double undral = -100.;
        double toler = 0.05;
        double slamin = 0.;
        ...
        for (int i = 0; i < step.getDimension(); i++) {
            if (Math.abs(step.getEntry(i)) < prec.eps()) {
                continue;
            }
            double ratio = Math.abs(st.vec().getEntry(i) / step.getEntry(i));
        ...
        double F0 = st.fval();
        double F1 = fcn.value(MnUtils.add(st.vec(), step));
