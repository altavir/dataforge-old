...
class PolynomialDistribution(
        ...
        val b: Double,
        val pow: Double,
        rng: RandomGenerator) : AbstractRealDistribution(rng) {

    val norm = 1.0 / (pow + 1) * (Math.pow(a, pow + 1) - Math.pow(b, pow + 1))
    ...
    override fun cumulativeProbability(x: Double): Double {
        return if (x < a) {
            ...
            (Math.pow(x, pow + 1) - Math.pow(a, pow + 1)) / (Math.pow(b, pow + 1) - Math.pow(a, pow + 1))
        } else {
            return 1.0
        }
    }
