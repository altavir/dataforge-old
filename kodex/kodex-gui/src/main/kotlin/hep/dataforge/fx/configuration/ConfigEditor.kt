 ...
 */
class ConfigEditor : BorderPane(), Initializable, Metoid {
    ...
    override fun meta(): Configuration? {
    ...
    private fun onValueAddClick(event: ActionEvent) {
        selectedItem.ifPresent { selected ->
            if (selected is ConfigFXNode) {
                ...
                if (valueName != null) {
                    selected.addValue(valueName, Value.NULL)
                }
            }
        }
    }
    ...
    companion object {
        ...
        fun build(configuration: Configuration, descriptor: NodeDescriptor): ConfigEditor {
            val component = ConfigEditor()
